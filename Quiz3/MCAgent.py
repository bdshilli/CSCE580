...
class MCAgent:
    ...
    def solve(self, initial_missionaries, initial_cannibals):
        ...
        def uniform_cost(): #uniform cost search
            inital_state = States(initial_missionaries, initial_cannibals, 0, 0, "left")
            if inital_state.goal_state():
                return inital_state
            priority_queue, cost, seen = [(0, inital_state)], {inital_state: 0}, set()
            while priority_queue:
                path_cost, state = inital_state.heappop(priority_queue)
                if state.goal_state(): return state
                if state in seen: continue
                seen.add(state)
                for n in successors(state):
                    new_path_cost = path_cost + 1
                    if n not in cost or new_path_cost < cost[n]:
                        cost[n] = new_path_cost; n.parent = state
                        heapq.heappush(priority_queue, (new_path_cost, n))
            return None
